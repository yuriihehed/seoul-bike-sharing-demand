...
/* Normality diagnostics: the project uses PROC UNIVARIATE with a normal
   Q-Q plot to check its regression residuals. The same check is run here
   on the response variable Rented_Bike_Count. */
title "Normal Q-Q Plot and Normality Tests for Rented Bike Count";
proc univariate data=bike_data normal;
    var Rented_Bike_Count;
    qqplot Rented_Bike_Count / normal;
run;
